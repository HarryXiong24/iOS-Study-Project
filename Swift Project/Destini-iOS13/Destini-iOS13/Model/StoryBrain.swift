...
struct StoryBrain {
    ...
    func getInitStory() -> Story {
        return self.stories[0]
    }
    
    mutating func getCurrentStory(_ currentChoice: String) -> Story {
        let currentStory = self.stories[currentStoryIndex]
        
        if currentChoice == currentStory.choice1 {
            currentStoryIndex = currentStory.choice1Destination
        } else {
            currentStoryIndex = currentStory.choice2Destination
        }
        
        return self.stories[currentStoryIndex]
    }
}
